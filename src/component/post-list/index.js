...
const Images = memo(
  ...
  (prevProps, nextProps) => prevProps.images && prevProps.images[0] === nextProps.images[0]
)
...
const Item = ({item, onPressPost, onPressFollow, isFollow, isNewProfile, showToast, signIn}) => {
    ...
    <TouchableOpacity onPress={onPress.bind()} activeOpacity={1}>
      <Card style={styles.postContainer}>
        ...
        {(!item.images || item.images.length === 0) && (
          <Request price={item.price || item.priceByMonth} area={item.area} />
        )}
        ...
        <BottomListPost
          onPressFollow={() => {
            if (isNewProfile) {
              showToast(signIn ? "Bạn chưa cập nhập thông tin" : "Bạn chưa đăng nhập")
            } else {
              setFollow(!follow)
