...
import newFeedNeedBuyService from "./service"

function* fetchPostNeedBuy(action) {
  const response = yield call(newFeedNeedBuyService.fetchPostNeedBuy, action.params)
  if (response.status === 200) {
    ...
    yield put(actions.fetchPostNeedBuyFailure(response.response ? response.response.data : {}))
  }
}
