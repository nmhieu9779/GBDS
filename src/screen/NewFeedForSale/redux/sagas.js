...
import newFeedForSaleService from "./service"

function* fetchPostForSale(action) {
  const response = yield call(newFeedForSaleService.fetchPostForSale, action.params)
  if (response.status === 200) {
    ...
    yield put(actions.fetchPostForSaleFailure(response.response ? response.response.data : {}))
  }
}
