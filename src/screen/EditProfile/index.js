...
const EditProfile = (props) => {
  ...
  useEffect(() => {
    props.uploadImageSuccess &&
      onEditProfile({avatarImageUrl: props.avatarImageUrl, email: props.email || email})
  }, [props.uploadImageSuccess])
  ...
  const handleResponse = async ({mime, path}) => {
    let formData = new FormData()
    formData.append("file", {
      uri: path,
      name: `avatar${editContentType(mime)}`,
      type: mime
    })
    formData.append(
      "uploadAs",
      `avatar/${props.id ? props.id : Date.parse(new Date())}${editContentType(mime)}`
    )
  ...
  const onEditProfile = (body) => {
    let tempBody = props.isNewProfile ? {...body, fieldx: newFieldx} : body
    props.editProfile({
      body: tempBody,
